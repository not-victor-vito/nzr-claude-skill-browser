...
// Shared helper — extract the authenticated user's identity from Easy Auth header
function getRequester(request) {
  ...
  try {
    ...
    if (Array.isArray(principal.claims)) {
      const nameTyp = principal.name_typ || 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/upn'
      const identity = principal.claims.find((c) => c.typ === nameTyp)
      const fallback = principal.claims.find(
        (c) =>
          c.typ === 'preferred_username' ||
          c.typ === 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress' ||
          c.typ === 'email',
      )
      return identity?.val || fallback?.val || principal.userId || null
    }
  } catch {}
  return null
}

// PUT /api/skills/:id — update title, icon, description, prompt, tags (owner only)
app.http('UpdateSkill', {
  ...
  handler: async (request, context) => {
    ...
    if (!requester) return json({ error: 'Could not identify requester.' }, 401)

    let body
    try { body = await request.json() } catch { return json({ error: 'Request body must be JSON.' }, 400) }

    const { title, icon, description, prompt, tags } = body
    ...
    if (prompt.length > 20000) return json({ error: 'prompt must be 20,000 characters or fewer.' }, 400)
    ...
    try {
      ...
      if (resource.submitted_by.toLowerCase() !== requester.toLowerCase()) {
        return json({ error: 'You can only edit your own skills.' }, 403)
      }

      resource.title = title.trim().slice(0, 100)
      resource.icon = typeof icon === 'string' ? icon.trim().slice(0, 8) : resource.icon
      resource.description = (description || '').toString().trim().slice(0, 500)
      resource.prompt = prompt.trim()
      ...
      context.error('UpdateSkill error:', err.message)
...
app.http('DeleteSkill', {
  ...
  handler: async (request, context) => {
    ...
    try {
      ...
      if (resource.submitted_by.toLowerCase() !== requester.toLowerCase()) {
        return json({ error: 'You can only delete your own skills.' }, 403)
      }

      await container.item(id, id).delete()
      return new Response(null, { status: 204 })
    } catch (err) {
      context.error('DeleteSkill error:', err.message)
...
app.http('PostSkill', {
  ...
  route: 'skills',
  handler: async (request, context) => {
    const submittedBy = getRequester(request) || 'unknown'
